...
-- Исправление: ScrollingFrame с правильным ZIndex и обработкой кликов
...
FunctionList.ZIndex = 1

-- ВАЖНО: отключаем перехват кликов
FunctionList.Selectable = false
...
-- ===== ДОБАВЛЕНИЕ ФУНКЦИЙ (ИСПРАВЛЕННОЕ) =====
local function AddFunction(name, description, callback)
    ...
    btn.ZIndex = 3 -- Важно: выше чем ScrollingFrame
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if not success then
            ...
            if player and player.Character then
                game:GetService("Chat"):Chat(player.Character, "❌ Ошибка: " .. tostring(err))
            end
        else
...
AddFunction("💰 Показать монеты", "Количество на карте", function()
    local count = 0
    for _, coin in pairs(workspace:GetDescendants()) do
        if coin:IsA("Part") and (coin.Name:lower():find("coin") or coin.Name:lower():find("money") or coin.Name:lower():find("gold")) then
            count = count + 1
...
AddFunction("📌 К убийце", "Телепорт к убийце", function()
    local player = game.Players.LocalPlayer
    for _, v in pairs(game.Players:GetChildren()) do
...
AddFunction("📌 К шерифу", "Телепорт к шерифу", function()
    local player = game.Players.LocalPlayer
    for _, v in pairs(game.Players:GetChildren()) do
        if v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v ~= player then
...
AddFunction("💀 Бессмертие", "God Mode", function()
...
AddFunction("⚡ Скорость x2", "Увеличение скорости", function()
    game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = 32
    game:GetService("Chat"):Chat(game.Players.LocalPlayer.Character, "⚡ Скорость x2")
end)

AddFunction("⚡ Скорость x3", "Ещё быстрее", function()
    game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = 48
    game:GetService("Chat"):Chat(game.Players.LocalPlayer.Character, "⚡ Скорость x3")
end)
...
AddFunction("📈 Прыжок x2", "Высокий прыжок", function()
    game.Players.LocalPlayer.Character.Humanoid.JumpPower = 100
end)

AddFunction("📈 Прыжок x3", "Очень высокий", function()
...
AddFunction("📦 Всё оружие", "Собрать всё оружие", function()
    for _, item in pairs(workspace:GetDescendants()) do
...
AddFunction("👁️ ESP (ВКЛ)", "Показать всех через стены", function()
    ESPEnabled = true
    for _, v in pairs(game.Players:GetChildren()) do
        if v.Character then
...
AddFunction("👁️ ESP (ВЫКЛ)", "Убрать ESP", function()
    ESPEnabled = false
    for _, v in pairs(game.Players:GetChildren()) do
        if v.Character then
...
AddFunction("🪟 Wallhack (ВКЛ)", "Прозрачные стены", function()
...
AddFunction("🪟 Wallhack (ВЫКЛ)", "Вернуть стены", function()
...
-- =============================================
-- ЗАПУСК
print("✅ Zoyka21 Hub v3.1 загружен! Нажмите на шестерёнку для открытия меню")
print("📱 Оптимизировано для телефонов")
print("🔧 Исправлена проблема с кликами по кнопкам")

game:GetService("Chat"):Chat(game.Players.LocalPlayer.Character, "📱 Zoyka21 Hub v3.1 загружен!")
